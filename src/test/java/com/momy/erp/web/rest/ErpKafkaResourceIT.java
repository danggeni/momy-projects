...
import com.momy.erp.config.KafkaProperties;
...
class ErpKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/erp-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/erp-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
